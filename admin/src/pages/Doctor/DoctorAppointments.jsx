        ...
        {appointments.map((item, index) => (
          <div className='flex flex-wrap justify-between max-sm:gap-5 max-sm:text-base sm:grid grid-cols-[0.5fr_2fr_1fr_1fr_3fr__1fr_1fr] gap-1 items-center text-gray-500 py-3 px-6 border-b hover:bg-gray-50' key={index}>
            ...
            {item.cancelled ? (
              ...
              <div className='flex'>
                <img onClick={() => handleCancelAppointment(item._id)} className='w-10 cursor-pointer' src={cancelIcon} alt="Cancel" />
                <img onClick={() => handleCompleteAppointment(item._id)} className='w-10 cursor-pointer' src={tickIcon} alt="Complete" />
              </div>
            )}
          </div>
        ))}
